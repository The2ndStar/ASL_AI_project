...
sc.register_shape("bottlecup.gif")
sc.register_shape("broc.gif")
sc.bgpic('bg.gif')
...
def over():
    global game_over, score, lives, bottles, brocs
    ...
    over_t.goto(0, 0)
    over_t.color("black")
    over_t.write("แล้วเต่าทะเลล่ะ", align="center", font=("Arial", 32, "bold"))

    # Wait for player input (Y to restart, W to quit)
    while True:
        ...
        if bboxes:
            ...
            if lmList:
                ...
                if len(test_point) == len(feature_cols):
                    ...
                    if predicted_label == 'Y':  # Restart the game
                        game_over = False
                        score = 0
                        lives = 3
                        t.goto(home_x, home_y)
                        for bottle in bottles:
                            spawn_particle(bottle, bottles + brocs, home_x, home_y)
                        for broc in brocs:
                            spawn_particle(broc, bottles + brocs, home_x, home_y)
                        update_display()
                        return  # Exit the over function and restart the game

                    elif predicted_label == 'W':  # Quit the game
                        cap.release()
                        cv2.destroyAllWindows()
                        sc.bye()  # Close the turtle screen
                        return  # Exit the over function and close the game
